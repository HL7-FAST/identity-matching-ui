      ...
      if (err instanceof HttpErrorResponse) {
        if (typeof err.error === 'string') {
          ...
          if (err.error.resourceType === 'OperationOutcome' && err.error.issue && Array.isArray(err.error.issue)) {
            errMsg = 'OperationOutcome error: ' + (err.error.issue || []).filter((issue: OperationOutcomeIssue) => issue.severity === 'error').map((issue: OperationOutcomeIssue) => issue.diagnostics).join(', ');
          }
          else {
